...
- Venue: **preprint**
- PDF trong repo: [docs/papers/long-horizon/seedpolicy_self_evolving_diffusion.pdf](../../../paper/long-horizon/seedpolicy_self_evolving_diffusion.pdf)
- Phân loại: **memory module**. Paper tự đối chiếu trực tiếp với
  [MemoryVLA](memoryvla.md) — xem mục 5.4.
...
So sánh với [MemoryVLA](memoryvla.md): công thức fusion **giống hệt về hình
...
Chế độ (1) chính là vấn đề số 2 của [../tong_quan.md](../tong_quan.md)
...
- **Đây là một trong các paper nhẹ nhất để tái lập trong corpus hiện tại.** Toàn bộ thí nghiệm
  ...
  số một — vượt cả [Seer](../future_prediction/seer.md) (65M, cần pretrain).
- **Bảng 5.4 lấp một lỗ hổng đã ghi ở mục 9 của [../tong_quan.md](../tong_quan.md)**:
...
- Với `vla-data-tools`: **không cần nhãn thêm gì cả**. Cùng nhóm nhẹ nhất với
  [MemoryVLA](memoryvla.md), [Seer](../future_prediction/seer.md) và
  [ACoT-VLA](../future_prediction/acot_vla.md).
...
3. **Đo latency và bộ nhớ của SEGA** để hoàn thiện bảng chi phí ở mục 9 của
   [../tong_quan.md](../tong_quan.md). Ta tự đo được vì có code và phần cứng
   phù hợp.
