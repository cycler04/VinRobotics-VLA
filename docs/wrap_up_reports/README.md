# vla_core — Mô hình VLA kết hợp bộ nhớ nhận thức - tư duy

Mô hình Vision-Language-Action được xây dựng trên VLM **Qwen3.5-0.8B** đóng băng, đi kèm với chuyên gia hành động (action expert) dạng flow-matching và ngân hàng bộ nhớ **PCMB theo kiểu MemoryVLA (tùy chọn)**.
Có hai hướng huấn luyện được triển khai ở đây:

1. **Pretraining tiền huấn luyện trên Corpus (Run-1)** — Không gian hành động con người - góc nhìn thứ nhất (human-ego) 153 chiều trên tập dữ liệu Layer-1 (repo anh em `data_corpus`; file `ACTION_SPEC.md` tại đó là hợp đồng nhãn).
2. **Benchmark bộ nhớ LIBERO-Long** — Hành động delta-EEF 7 chiều trên các demo `libero_10 + libero_90`, được đánh giá bằng tỷ lệ thành công của bộ mô phỏng trên bộ bài test `libero_10`. Nhánh điều khiển không dùng bộ nhớ nằm ở repo anh em `vla_core-benchmark-libero-base/` (xem `docs/benchmark_protocol.md` tại đó).

## Mô hình (Model)

```
Qwen3.5-0.8B (VLM đóng băng)
...
Hàm mất mát kép (Dual loss): flow-MSE (chuyên gia hành động) + LM narrative có trọng số (backbone/LoRA).
detach_action_input=True giúp giữ hai hàm loss này trên các tham số tách biệt.
```

Tổng kiểm kê: 1.154B thông số, 555M thông số có thể huấn luyện dưới cấu hình mặc định của corpus; cấu hình LIBERO (đóng băng hoàn toàn backbone, tắt narrative) sử dụng ~290M (head + memory).
Chi tiết: `docs/memory_module/model_parameters.md`.

### Ý tưởng về bộ nhớ

Mô hình cơ sở là loại đơn khung hình — nó không thể phân biệt các trạng thái giống hệt nhau về mặt thị giác trước và sau một hành động. Chúng tôi đưa Ngân hàng Bộ nhớ Nhận thức - Tư duy (PCMB) của MemoryVLA (ICLR 2026) vào giai đoạn *pretraining*, chứ không phải một giải pháp bổ sung sau khi finetune:

- Một `MemoryBank` cho mỗi vị trí batch; các đặc trưng hợp nhất (sau khi retrieval + gate) được push và mặc định là detach; hợp nhất gộp lân cận khi đạt sức chứa tối đa (L=16).
- Ngân hàng bộ nhớ duy nhất từ lớp ẩn cuối cùng, được broadcast đến tất cả 24 khối head thông qua một luồng bộ nhớ có cổng chuyên dụng (các gate bắt đầu từ 0 và không bao giờ bị weight-decay).
- **Group mode** (`EpisodeGroupBatchSampler`): batch = K group × G=16 window sắp xếp theo thời gian của một clip; bank được xây dựng đệ quy trong phạm vi group (bám sát bài báo gốc).
...
Nhánh mô hình cơ sở (không dùng bộ nhớ) đạt **16.4%** trên LIBERO-Long; mô hình dùng bộ nhớ hiện ở mức **3.4%** — bị nghẽn do lỗi cấu hình lịch trình học tập (learning-rate schedule), cần huấn luyện lại trước khi kết quả so sánh có hiệu lực. Báo cáo đầy đủ: `docs/progress_report_2026-08-24.md`.
...
- Giấy phép xp10m là KHÔNG THƯƠNG MẠI — áp dụng cho các trọng số được huấn luyện trên đó.
