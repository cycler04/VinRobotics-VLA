...
- **Paper có mục "Limitations & Future Works".** Tác giả nêu chi phí triển khai/
  tài nguyên và giới hạn của action chunk representation; không nên nói paper bỏ
  qua hoàn toàn giới hạn.
- **Paper có latency analysis**: baseline 91 ms, thêm EAR 110 ms, IAR thêm khoảng
  2 ms. Tuy vậy, report không đủ chi tiết phần cứng/cấu hình để so trực tiếp với
  các paper khác; latency độc lập vẫn cần đo lại trước khi ra quyết định tích hợp.
...
- **Missing baseline**: không so với các hệ manager tách rời dù cùng dùng π0.5 làm
  nền và cùng đo trên VLABench.
