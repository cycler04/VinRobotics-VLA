...
- PDF trong repo: [docs/papers/05-long-horizon/13_causal_world_modeling_lingbot.pdf](../../../papers/05-long-horizon/13_causal_world_modeling_lingbot.pdf)
...
Đây là **cùng một phát hiện** với MemoryVLA++ ("1 bước denoise là đủ") tìm ra độc
lập — xem [../memory_modules/02_memoryvla_pp.md](../memory_modules/02_memoryvla_pp.md).
...
memory trong toàn bộ 15 paper — task được thiết kế sao cho baseline có giá trị kỳ
...
- Baseline chính chỉ là π0.5 và Motus. Không so với
  [MemoryVLA++](../memory_modules/02_memoryvla_pp.md) dù cả hai đều là world model
  + memory, cùng tuyên bố về task đếm/nhớ.
...
- Bốn cách dùng world model nay đã rõ ràng thành một phổ, xem bảng ở mục 7 của
  [../memory_modules/02_memoryvla_pp.md](../memory_modules/02_memoryvla_pp.md).
  LingBot-VA nằm ở cực "video và action trong cùng một chuỗi", khác hẳn cực
  "sinh ảnh rời rồi phản tỉnh" của
  [ReflectVLM](02_reflective_planning.md).
- Hai phát hiện có thể tách ra dùng ngay, độc lập kiến trúc:
  1. **Partial denoising** — action không cần video đã khử nhiễu hết. Xác nhận
     độc lập bởi MemoryVLA++.
...
2. **Quét $s_{aug}$ / mức partial denoise** trên checkpoint công khai: $s \in
   ...
   hỏi chi phí/chất lượng mà cả LingBot-VA lẫn MemoryVLA++ đều bỏ ngỏ.
3. **Đo latency tuyệt đối** của checkpoint công khai trên GPU consumer, đối chiếu
   với 0.241 s của MemoryVLA++ và 73 ms của
