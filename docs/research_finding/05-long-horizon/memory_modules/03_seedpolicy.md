...
- Venue: **preprint**
- PDF trong repo: [docs/papers/05-long-horizon/15_seedpolicy_self_evolving_diffusion.pdf](../../../papers/05-long-horizon/15_seedpolicy_self_evolving_diffusion.pdf)
...
**Đây là bảng có giá trị nhất của paper và hiếm nhất trong cả 15 paper**: ba cơ chế
memory khác nhau, cùng backbone DP, cùng giao thức huấn luyện. Không paper nào
khác trong tập làm đối chứng cơ chế kiểu này.
...
- **Đây là paper rẻ nhất để tái lập trong toàn bộ 15 paper.** Toàn bộ thí nghiệm
...
- Với `vla-data-tools`: **không cần nhãn thêm gì cả**. Cùng nhóm nhẹ nhất với
  ...
  [ACoT-VLA](../future_prediction/05_acot_vla.md).
...
1. **Tái lập đường cong horizon scaling** (Fig. 1) trên code công khai: đo hiệu
   ...
   cả tập.
2. **Quét $N_s$** — chưa ai quét. Đối chiếu với nghịch lý độ dài memory $L$ của
   [MemoryVLA++](02_memoryvla_pp.md) (tối ưu phụ thuộc task: 16 cho SimplerEnv,
   256 cho Real-Temporal). Nếu $N_s$ cũng phụ thuộc task theo cùng kiểu thì đó là
   quy luật chung của memory bank, không phải đặc thù một kiến trúc.
