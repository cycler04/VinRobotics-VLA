...
- PDF trong repo: [docs/papers/06-retry-handle/02_failsafe_reasoning_recovery_from_failures.pdf](../../../papers/06-retry-handle/02_failsafe_reasoning_recovery_from_failures.pdf)
...
- Phân loại: **failure recovery data** — can thiệp ở tầng *dữ liệu* (sinh cặp
  failure–action) cộng một *external assistant* chạy song song VLA lúc deploy.
...
- FailSafe là **đối lập trực tiếp của RaC** ([05-long-horizon/recovery_data](../../05-long-horizon/recovery_data/01_rac.md)):
